...
//				verbose=Tools.parseBoolean(b); //123
				...
				xMinLoc=(int)Tools.parseKMG(b);
			}else if(a.equals("miny") || a.equals("ymin")){
				yMinLoc=(int)Tools.parseKMG(b);
			}else if(a.equals("maxx") || a.equals("xmax")){
				xMaxLoc=(int)Tools.parseKMG(b);
			}else if(a.equals("maxy") || a.equals("ymax")){
				yMaxLoc=(int)Tools.parseKMG(b);
			}
...
//			out2=null;
	...
	public void process(){
		...
		if(varFile!=null || vcfFile!=null){
			...
			fixVariants=(varMap!=null && varMap.size()>0 && scafMap!=null && scafMap.size()>0);
		}
...
//		Shared.READ_BUFFER_LENGTH=rbl0;
	...
	public static void putRqc(String key, long value, boolean evict){putRqc(key, value+"", evict);}
		...
		public void run(){
			...
			while(reads!=null && reads.size()>0){
				...
				for(int i=0; i<reads.size(); i++){
					...
					if(locationFilter && !r1.discarded()){
					...
					if(r1.discarded()){
						badHeaderBasesT+=initialLength1+initialLength2;
						badHeaderReadsT+=1+r1.mateCount();
					}
...
//			System.err.println("entropy = "+(sum*entropyMult));
		...
		private long badHeaderBasesT=0;
		private long badHeaderReadsT=0;
	}
	...
	private int xMinLoc=-1;
	private int yMinLoc=-1;
	private int xMaxLoc=-1;
	private int yMaxLoc=-1;
