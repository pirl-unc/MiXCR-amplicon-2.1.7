...
public class TaxServer {
	...
	public TaxServer(String[] args) throws Exception {
		...
		sketchThreads=Tools.min(16, Shared.threads());
		ArrayList<String> refSketchNames=new ArrayList<String>();
		...
		for(int i=0; i<args.length; i++){
			...
			if(parser.parse(arg, a, b)){//Parse standard flags in the parser
				...
				refSketchNames.add(arg);
			}else{
				throw new RuntimeException(arg);
			}
		}
		
		sketchThreads=Shared.threads();
		...
		if(!refSketchNames.isEmpty()){
			outstream.println("Loading sketches.");
			refSketches=tool.loadSketches_MT(SketchObject.PER_TAXA, refSketchNames);
		}
...
//		httpServer.createContext("/help", new HelpHandler());
	...
	class SketchHandler implements HttpHandler {
		...
		public void handle(HttpExchange t) throws IOException {
			...
			if(fileMode){
				sketches=tool.loadSketches(rparam, null, SketchObject.ONE_SKETCH);
			}else{
				InputStream is=t.getRequestBody();
				String s=readStream(is);
				if(s!=null && s.length()>0){
					sketches=tool.loadSketches(s);
				}
			}
...
//					System.err.println(j);
	...
	public void compare(ArrayList<Sketch> sketches, StringBuilder sb){
		...
		for(int i=0; i<sketches.size(); i++){
			...
			if(sketchThreads<2){
				...
				ArrayList<CompareThread> alct=new ArrayList<CompareThread>(sketchThreads);
				for(int t=0; t<sketchThreads; t++){
					alct.add(new CompareThread(a, t, sketchThreads, fakeID, map));
				}
	...
	private void writeResults(ArrayList<Comparison> al, Sketch s, StringBuilder sb){
		...
		for(Comparison c : al){
			...
			if(format==0){
				...
				if(tree!=null && c.taxID>=0 && c.taxID<minFakeID){
					TaxNode tn=tree.getNode(c.taxID);
	...
	private boolean processPair(Sketch a, Sketch b, int[] buffer, AtomicInteger fakeID, ConcurrentHashMap<Integer, Comparison> map){
		Comparison c=compareOneToOne(a, b, buffer, minCount, minID, null);
		if(c==null){return false;}
		if(c.taxID<0){c.taxID=fakeID.getAndIncrement();}
		
		TaxNode tn=(tree==null ? null : tree.getNode(b.taxID));
		if(tn!=null){
			c.name=tn.name;
			if(tn.level<sketchTaxLevel){
				tn=tree.getNodeAtLevel(b.taxID, sketchTaxLevel);
			}
		}
...
//		System.err.print("*");
	...
	private int sketchSize=2000;
	private final int k=31;
	private final boolean rcomp=true;
	private final long minFakeID=1900000000;
	private final int maxRecords=100;
	private final float minID=0.0002f;
	private final int minCount=3;
	private int format=0;
	int sketchTaxLevel=TaxTree.SPECIES;
	private final SketchTool tool=new SketchTool(sketchSize, k, minCount, rcomp);
	private ArrayList<Sketch> refSketches=new ArrayList<Sketch>();
	
	private int sketchThreads=16;
