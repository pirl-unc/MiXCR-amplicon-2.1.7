...
public class PrintTaxonomy {
	...
	public PrintTaxonomy(String[] args){
		...
		taxLevelExtended=taxLevel<0 ? taxLevel : TaxTree.levelToExtended(taxLevel);
		minLevelExtended=minLevel<0 ? minLevel : TaxTree.levelToExtended(minLevel);
		maxLevelExtended=maxLevel<0 ? maxLevel : TaxTree.levelToExtended(maxLevel);
...
//		tsw.print("\n");
	...
	StringBuilder makeTaxLine(TaxNode tn, int minLevel, int maxLevel){
		...
		final int minLevelE=minLevel<0 ? minLevel : TaxTree.levelToExtended(minLevel);
...
//		tsw.print("\n");
	...
	void printTaxLevel(TaxNode tn, final TextStreamWriter tsw){
		if(tn==null){tn=unknown;}
		while(tn!=null && tn.id!=tn.pid && tn.levelExtended<taxLevelExtended){tn=tree.getNode(tn.pid);}
		if(tsw!=null)tsw.println(tn.name);
		tn.incrementRaw(1);
	}
