...
public class VarFilter {
	...
	public boolean passesFilter(Var v, float pairingRate, float totalQualityAvg, float totalMapqAvg, float readLengthAvg, int ploidy, ScafMap map){
...
//		if(minAvgMapq>0 && v.mapQAvg()<minAvgMapq){return false;}
		...
		//assert(v.coverage()>0);
		if(v.coverage()>0 && minAlleleFraction>0){
			final float af=v.revisedAlleleFraction==-1 ? v.alleleFraction() : v.revisedAlleleFraction;
			if(af<minAlleleFraction){return false;}
		}
		
		if(minScore>0 && v.phredScore(pairingRate, totalQualityAvg, totalMapqAvg, readLengthAvg, rarity, ploidy, map)<minScore){return false;}
