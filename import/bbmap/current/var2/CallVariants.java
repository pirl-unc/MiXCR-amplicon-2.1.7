...
//		System.err.println(properlyPairedReadsProcessed+", "+readsProcessed+", "+readsDiscarded);
		...
		varMap.readLengthAvg=trimmedBasesProcessed/(float)Tools.max(1, readsProcessed-readsDiscarded);
		t2.stop("Time: ");
		outstream.println();
...
//		assert(false) : filter.toString(properPairRate, ploidy);
		...
		if(ffout!=null || vcf!=null){
			...
			if(ffout!=null){
				varMap.writeVarFile(ffout, filter);
			}
			if(vcf!=null){
				varMap.writeVcfFile(vcf, filter, sampleName, readsProcessed-readsDiscarded, 
						pairedInSequencingReadsProcessed, properlyPairedReadsProcessed, trimmedBasesProcessed, ref, trimWhitespace);
			}
	...
	private long[] processVariants(){
		return varMap.processVariantsMT(filter, scoreArray);
	}
	...
	static boolean writeScoreHist(String fname, long[] array){
		...
		tsw.println("#Mean\t"+String.format("%.2f", sum2*1.0/sum));
