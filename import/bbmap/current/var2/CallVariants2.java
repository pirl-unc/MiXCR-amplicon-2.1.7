...
//		assert(false) : sampleNames;
		...
		for(Sample sample : samples){
			...
			if(sample.vcfName!=null){
				sample.varMap.writeVcfFile(sample.vcfName, filter, sample.name, sample.readsProcessed-sample.readsDiscarded,
						sample.pairedInSequencingReadsProcessed, sample.properlyPairedReadsProcessed, sample.trimmedBasesProcessed, ref, trimWhitespace);
			}
	...
	class Sample{
		...
		public long process1(VarMap sharedVarMap) {
			...
			final float readLengthAvg=trimmedBasesProcessed/(float)Tools.max(1, readsProcessed-readsDiscarded);
...
//			assert(false) : filter.toString(properPairRate, ploidy);
		...
		private long[] processVariants(){
			return varMap.processVariantsMT(filter, null);
		}
