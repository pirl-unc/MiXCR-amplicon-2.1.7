...
//		assert(reflen==readlen) : reflen+", "+readlen+", "+new String(allele)+", "+start+", "+stop;
	...
	public static String toHeader(float properPairRate, float totalQualityAvg, float mapqAvg, float readLengthAvg, float minAlleleFraction, float rarity, int ploidy){
		StringBuilder sb=new StringBuilder();
		sb.append("#ploidy\t"+ploidy+"\n");
		sb.append(String.format("#properPairRate\t%.4f\n", properPairRate));
		sb.append(String.format("#totalQualityAvg\t%.4f\n", totalQualityAvg));
		sb.append(String.format("#mapqAvg\t%.2f\n", mapqAvg));
		sb.append(String.format("#readLengthAvg\t%.2f\n", readLengthAvg));
		sb.append(String.format("#minAlleleFraction\t%.4f\n", minAlleleFraction));
	...
	public ByteBuilder toText(ByteBuilder bb, float properPairRate, float totalQualityAvg, float totalMapqAvg, float readLengthAvg, float rarity, int ploidy, ScafMap map){
		useIdentity=true;
...
//		bb.append(prevBase<0 ? 'N' : (char)prevBase).append('\t');
		...
		final float score=score(properPairRate, totalQualityAvg, totalMapqAvg, readLengthAvg, rarity, ploidy, map);
		bb.append(String.format("%.2f", toPhredScore(score))).append('\t');
		
		if(extendedText){
			
			bb.append(count()).append('\t');
			float af=alleleFraction();
			bb.append(String.format("%.4f\t", af));
			bb.append(String.format("%.4f\t",revisedAlleleFraction(af, readLengthAvg)));
			bb.append(String.format("%.4f\t",strandRatio()));
			bb.append(String.format("%.2f\t",baseQAvg()));
			bb.append(String.format("%.2f\t",mapQAvg()));
			bb.append(String.format("%.2f\t",edistAvg()));
			bb.append(String.format("%.2f\t",identityAvg()));
			
			bb.append(String.format("%.4f\t",edistScore()));
			bb.append(String.format("%.4f\t",identityScore()));
			bb.append(String.format("%.4f\t",qualityScore(totalQualityAvg)));
			bb.append(String.format("%.4f\t",pairedScore(properPairRate, scafEndDist)));
			bb.append(String.format("%.4f\t",biasScore(properPairRate, scafEndDist)));
			bb.append(String.format("%.4f\t",coverageScore(ploidy, rarity, readLengthAvg)));
			bb.append(String.format("%.4f\t",homopolymerScore(map)));
			bb.append(String.format("%.4f\t",score));
		}
	...
	public static String toVcfHeader(float properPairRate, float totalQualityAvg, float mapqAvg, float rarity, float minAlleleFraction, int ploidy, 
			long reads, long pairs, long properPairs, long bases, ScafMap map, String sampleName, String ref, boolean trimWhitespace) {
		StringBuilder sb=new StringBuilder();
		...
		sb.append(String.format("##readLengthAvg=%.3f\n", (bases/Tools.max(reads, 1.0))));
		...
		{	
			sb.append("##FORMAT=<ID=FAIL,Description=\"Fail\">\n");
			sb.append("##FORMAT=<ID=PASS,Description=\"Pass\">\n");
	...
	public ByteBuilder toVCF(ByteBuilder bb, float properPairRate, float totalQualityAvg, float mapqAvg, float readLengthAvg, 
...
//			final int covMinus=(Scaffold.trackStrand ? scaf.minusCoverage(this) : coverage/2);
			...
			final int refMinus=Tools.max(0, covMinus-minusCount());
			final int refPlus=Tools.max(0, covPlus-plusCount());
			...
			float af=alleleFraction();
			bb.append("AF=").append(String.format("%.4f",af)).append(';');
			bb.append("RAF=").append(String.format("%.4f", revisedAlleleFraction(af, readLengthAvg))).append(';');
	...
	public float phredScore(float properPairRate, float totalQualityAvg, float totalMapqAvg, float readLengthAvg, float rarity, int ploidy, ScafMap map){
		float score=score(properPairRate, totalQualityAvg, totalMapqAvg, readLengthAvg, rarity, ploidy, map);
		return toPhredScore(score);
	}
	
	public float score(float properPairRate, float totalQualityAvg, float totalMapqAvg, float readLengthAvg, float rarity, int ploidy, ScafMap map){
		int scafEndDist=(map==null ? start : contigEndDist(map));
		
		float cs=coverageScore(ploidy, rarity, readLengthAvg);
		if(cs==0){return 0;}
		float es=(useEdist ? edistScore() : 1);
		float qs=qualityScore(totalQualityAvg);
		float ps=(usePairing ? pairedScore(properPairRate, scafEndDist) : 1);
		float bs=(useBias ? biasScore(properPairRate, scafEndDist) : 1);
		float is=(useIdentity ? identityScore() : 1);
		float hs=(useHomopolymer ? homopolymerScore(map) : 1);
	...
	public float pairedScore(float properPairRate, int scafEndDist){
		if(properPairRate<0.5){return 0.98f;}
		final float count=count();
		if(count==0){return 0;}
		float rate=properPairCount/count;
		rate=rate*(count/(0.1f+count));
		if(rate*1.05f>=properPairRate){return Tools.max(rate, 1-0.001f*properPairRate);}
		float score=((rate*1.05f)/properPairRate)*0.5f+0.5f;
		score=Tools.max(0.1f, score);
		return modifyByEndDist(score, scafEndDist);
	}
	
	public float modifyByEndDist(float x, int scafEndDist){
		if(x>=0.99 || !doNscan || scafEndDist>=nScan){return x;}
		if(scafEndDist<minEndDistForBias){return Tools.max(x, 0.98f+0.02f*x);}
		float delta=1-x;
		delta=delta*(scafEndDist*scafEndDist)/(float)(nScan*nScan);
		return 1-delta;
	}
	
	public float coverageScore(int ploidy, float rarity, float readLengthAvg){
		int count=count();
		if(count==0){return 0;}
...
//		float ratio=alleleFraction();
	...
	public void reviseAlleleFraction(float readLengthAvg, Scaffold scaffold, VarMap map){
		assert(type()==INS);
		...
		final float af=alleleFraction();
		final float raf=revisedAlleleFraction(af, readLengthAvg);
		final float revisedDif=0.55f*(raf-af); //Half on the left and half on the right, on average
		final float mult=revisedDif/allele.length;
		
		for(int i=0, j=start; i<allele.length && j<scaffold.bases.length; i++, j++){
			final byte b=allele[i];
			if(b!=bases[j]){
				Var key=new Var(scaffold.number, j, j+1, b);
				Var value=map.get(key);
//				System.err.println("At pos "+j+": ref="+(char)bases[j]+", alt="+(char)b);
				if(value!=null){
//					System.err.println("Found "+value);
					synchronized(value){
						if(value.revisedAlleleFraction==-1){
							float sub=revisedDif-mult*i;
							float old=value.alleleFraction();
//							System.err.println("sub="+sub+", old="+old);
							value.revisedAlleleFraction=Tools.max(old*0.05f, old-sub);
						}
					}
				}
			}
	...
	public float revisedAlleleFraction(float af, float readLengthAvg){
	...
	public float adjustForInsertionLength(final float ratio, final float rlen0){
		if(type()!=INS){return ratio;}
		final int ilen=readlen();
		if(ilen<2){return ratio;}
		
		final float rlen=Tools.max(ilen*1.2f+6, rlen0);
		final float sites=rlen+ilen-1;
		final float goodSites=rlen-ilen*1.1f-6;
		
		final float expectedFraction=goodSites/sites;
		final float revisedRatio=Tools.min(ratio/expectedFraction, 1-(1-ratio)*0.1f);
		return revisedRatio;
	}
	
	public float homopolymerScore(ScafMap map){
...
//	public static int homopolymerCountSubIns(final byte[] bases, final int pos, final byte base){
//		System.err.println("A:"+pos+", "+bases.length+", "+(char)base);
//		if(pos<0 || pos>=bases.length){return 0;}
//		if(!AminoAcid.isFullyDefined(base)){return 0;}
//		System.err.println("B:"+pos+", "+bases.length);
...
//		
//		int count2=0;
//		for(int i=pos+1, lim=Tools.min(bases.length, pos+4); i<lim; i++){
//			if(bases[i]==base){count2++;}
//			else{break;}
//		}
//		System.err.println("D:"+pos+", "+bases.length+", "+count2);
...
//	}
	...
	public float biasScore(float properPairRate, int scafEndDist){
		float strandBias=strandBiasScore(scafEndDist);
		float readBias=readBiasScore(properPairRate);
		return (float)Math.sqrt(strandBias*readBias);
	}
	
	public float strandBiasScore(int scafEndDist){
...
//	static final int NOCALL=0, SUB=1, DEL=2, INS=3;
	...
	static final String[] typeArray=new String[] {"INS","NOCALL","SUB","DEL"};
	static final int INS=0, NOCALL=1, SUB=2, DEL=3;
