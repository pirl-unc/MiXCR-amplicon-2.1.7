...
dupedist=40         (dist) Max distance to consider for optical duplicates.
                    ...
                    This is platform-specific; recommendations:
                       NextSeq      40  (and spantiles=t)
                       HiSeq 1T     40
                       HiSeq 2500   40
                       HiSeq 3k/4k  2500
spantiles=f         Allow reads to be considered optical duplicates if they
                    are on different tiles, but are within dupedist in
                    either the x or y axis.  Should only be enabled when
                    looking for tile-edge duplicates (as in NextSeq).
